...
class JwtAuthenticationFilterTest {
    ...
    @Test
    void shouldNotFilter_stripsContextPathBeforeCheck() {
        MockHttpServletRequest req = new MockHttpServletRequest();
    ...
    @Test
    void shouldNotFilter_normalizesLeadingSlash_forNonAuthPaths() {
        MockHttpServletRequest req = new MockHttpServletRequest();
    ...
    @Test
    void doFilterInternal_nonBearer_continuesChain() throws Exception {
        MockHttpServletRequest req = new MockHttpServletRequest();
        req.addHeader("Authorization", "Basic x");
        MockHttpServletResponse res = new MockHttpServletResponse();
    ...
    void doFilterInternal_invalidJwt_returns401() throws Exception {
        ...
        MockHttpServletResponse res = new MockHttpServletResponse();
        when(jwtService.extractUsername("bad")).thenThrow(new JwtException("bad"));
    ...
    @Test
    void doFilterInternal_loadUserThrows_returns401() throws Exception {
        MockHttpServletRequest req = new MockHttpServletRequest();
        req.addHeader("Authorization", "Bearer token");
        MockHttpServletResponse res = new MockHttpServletResponse();

        when(jwtService.extractUsername("token")).thenReturn("u@example.com");
        when(userDetailsService.loadUserByUsername("u@example.com")).thenThrow(new RuntimeException("missing"));

        filter.doFilterInternal(req, res, filterChain);

        assertThat(res.getStatus()).isEqualTo(401);
        assertThat(res.getContentAsString()).contains("User not found");
        verify(filterChain, never()).doFilter(req, res);
    }

    @Test
    void doFilterInternal_existingAuthentication_skipsUserLoad() throws Exception {
        MockHttpServletRequest req = new MockHttpServletRequest();
        req.addHeader("Authorization", "Bearer token");
        MockHttpServletResponse res = new MockHttpServletResponse();
