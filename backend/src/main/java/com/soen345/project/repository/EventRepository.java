...
public interface EventRepository extends JpaRepository<Event, Long> {
    ...
    @Query("SELECT e FROM Event e WHERE e.eventDate > :now AND " +
            "(LOWER(e.title) LIKE LOWER(CONCAT('%', :keyword, '%')) OR " +
            "LOWER(e.description) LIKE LOWER(CONCAT('%', :keyword, '%')))")
    List<Event> searchEventsByKeyword(@Param("keyword") String keyword, @Param("now") LocalDateTime now);

    // Filter events by category
    @Query("SELECT e FROM Event e WHERE e.eventDate > :now AND e.categoryId = :categoryId")
    List<Event> findByCategory(@Param("categoryId") Long categoryId, @Param("now") LocalDateTime now);

    // Filter events by location
    @Query("SELECT e FROM Event e WHERE e.eventDate > :now AND e.locationId = :locationId")
